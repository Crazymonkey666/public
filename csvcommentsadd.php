...
$query = $wpdb->prepare( "SELECT * FROM ".$wpdb->prefix."posts WHERE post_title = %s and post_type='product'", $data[6] );
$results = $wpdb->get_row( $query );
           
              if($results){
                $commentdata = array(
                    'comment_post_ID' => $results->ID, // 产品ID
            
                    'comment_author' => $data[4], // 评论作者
                    'comment_author_email' => $data[5], // 评论作者邮箱
                    'comment_content' => $data[0], // 评论内容
                    'comment_type' => 'review',
                    'comment_parent' => 0,
                    'user_id' => 0,
                    'comment_author_IP' => '',
